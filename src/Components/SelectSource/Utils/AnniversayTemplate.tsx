...
const AnniversaryTemplate = ({
  ...
  useEffect(() => {
    ...
    // if (appSettings?.AnnivTempImage) {
      setimagePreviewAnniAnni(imagesList.AnnivTempImage);
    // }
  }, []);
  ...
  async function handleSaveAnni() {
    ...
    updateSettingJson(SETTING_LIST, {...appSettings,AnniversaryEmailTemplate: editorAnniValue});

    updateSettingJson(IMAGES_LIST, {...imagesList,AnnivTempImage:base64Image})
    setAppSettings({...appSettings,AnniversaryEmailTemplate: editorAnniValue});
    setImagesList({...imagesList,AnnivTempImage:base64Image});
  ...
  return (
    <div id="emailTemplate">
      ...
      >
        ...
        <PrimaryButton
          text="Upload Image"
          onClick={() => fileInputRefAnni.current.click()}
          style={{
            background: "#fff",
            color: "#000",
      ...
      <div style={{ marginTop: "20px" }}>
        ...
        <div className={EdStyles.Qillplaceholdes}>
          <button
            ...
            onClick={() => insertPlaceholderAnni("[FirstName]")}
          >
            First Name
          </button>
          <button
            className={EdStyles.QuillPlaceholderBtn}
            onClick={() => insertPlaceholderAnni("[DOB]")}
          >
            DOB
          </button>
          <button
            className={EdStyles.QuillPlaceholderBtn}
            onClick={() => insertPlaceholderAnni("[DOJ]")}
          >
            DOJ
          </button>
          <button
            className={EdStyles.QuillPlaceholderBtn}
            onClick={() => insertPlaceholderAnni("[Department]")}
          >
            Department
          </button>
          <button
            className={EdStyles.QuillPlaceholderBtn}
            onClick={() => insertPlaceholderAnni("[JobTitle]")}
          >
            Job Title
          </button>
          <button
            className={EdStyles.QuillPlaceholderBtn}
            onClick={() => insertPlaceholderAnni("[DisplayName]")}
          >
            Display Name
          </button>
        </div>
