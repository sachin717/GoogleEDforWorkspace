...
function HideManagerDelete(props: any) {
  let allItems: any = [];
  React.useEffect(() => {
    GetSettingData();
  }, [allItems.length]);
  ...
  async function GetSettingData() {
    ...
    if (exists) {
      ...
      empisadmin = parsedData.HideManager;
  ...
  async function includedelete() {
    ...
    if (excluderoles.length == 0) {
      props.SweetAlertHideManager("Please select users to remove!");
      ...
      for (var y = 0; y < excluderoles.length; y++) {
        ...
        // }
        // else{
        //   setAlertMsg(props.langcode.s8?props.langcode.s8:"You can not remove yourself from the role");
        //   setAlert(true);
        //   setTimeout(() =>{
        //     messageDismiss();
        //   }, 5000);
      ...
      parsedData["HideManager"] = roleUser;
      ...
      await containerClient
        ...
        .then(() => {
          props.SweetAlertHideManager("success",translation.SettingSaved);
          // setSaved(true);
          setusers([]);
    ...
    <div>
      ...
      <div className="settingBlock">
        ...
        <div style={{ marginBottom: "2%", paddingTop: "1%" }}>
          <PrimaryButton text={"Delete user"} onClick={includedelete} />
        </div>
      </div>
...
export default HideManagerDelete;
