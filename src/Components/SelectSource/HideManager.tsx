...
let optionList: any = [];
function HideManager(props: any) {
  React.useEffect(() => {
    GetSettingData();
  }, [allItems.length]);
  ...
  async function addrole() {
    var rolesUser = "";
    if (users.length == 0) {
      props.SweetAlertHideManager("success","Please enter one user");
    ...
    parsedData["HideManager"] = rolesUser;
    ...
    if (text.target.value.length > 3) {
      getAllUsersInOrg(text.target.value).then((item: any) => {
        ...
        let staffMember = item.map((user: any) => {
          if (user.name.givenName != null && user.name.givenName != undefined) {
