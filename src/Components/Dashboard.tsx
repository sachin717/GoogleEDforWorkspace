...
const Dashboard = ({
  ...
  React.useEffect(() => {
    ...
    if (appSettings?.SyncUserInfoFrom == "Google & Imported User") {
      ...
      employees = usersList?.AllUsersData?.GoogleUsers??allUsers;
    }else{
      setSearchBoxData(usersList?.AllUsersData?.NonGoogleUser??[]);
      console.log("ss",usersList?.AllUsersData)
      employees = usersList?.AllUsersData?.NonGoogleUser??[];
    }
    // employees=[...users]
  ...
  const updateRecord = async () => {
    ...
    if (appSettings?.SyncUserInfoFrom == "Google & Imported User") {
      let user = usersList?.AllUsersData?.GoogleUsers?.find((item) => {
        return item?.email == selectedEmail;
      });
      let userIndex = usersList?.AllUsersData?.GoogleUsers?.findIndex(
        (item) => {
          return item?.email == selectedEmail;
        }
      );
      ...
      if (userIndex > -1) {
        let data = [...usersList?.AllUsersData?.GoogleUsers];
  ...
  function calculateGenderDiversityRatio(employees) {
    ...
    if (totalEmployees > 0) {
        maleRatio = ((genderCount["Male"] || 0) / totalEmployees * 100).toFixed(2);
        femaleRatio = ((genderCount["Female"] || 0) / totalEmployees * 100).toFixed(2);
    }
  ...
  useEffect(() => {
    const departmentEmployeeCounts = aggregateEmployeesByDepartment(employees)??0;
    const departmentChartData = formatChartData(departmentEmployeeCounts);
    setTotalEmpDeptData(departmentChartData);

    const genderDistributionByLocation = aggregateGenderByLocation(employees)??0;
  ...
  const updateBulkUsers = (updates: UpdateField[]) => {
    if (!usersList?.AllUsersData?.GoogleUsers) {
      console.error("User data not available.");
      return;
    }
    let users = [...usersList.AllUsersData.GoogleUsers];
  ...
  const saveDataBulkUsers = () => {
    ...
    if (updateDataArray?.length) {
      ...
      let data = {
        GoogleUsers: results,
        NonGoogleUser: usersList?.NonGoogleUser,
      };
      updateSettingJson(USER_LIST,encryptData(JSON.stringify({ ...usersList, AllUsersData: data })));
      setUsersList({ ...usersList, AllUsersData: data });
      SweetAlertDashboard("success", translation.SettingSaved);
    } else {
      ...
      >
        <div style={containerStyle}>
          <div className="chartHeaders" style={{...itemStyle}}>
            ...
            <p>
              {translation.Head ? translation.Headcount : "Count Head "}
            </p>
          </div>
          <div className="chartHeaders" style={{...itemStyle,background:"#ff7f2b"}}>
            <p>{sampleData ? "150" : fullTimeCount}</p>
            <p>{translation?.FullTime ? translation?.FullTime : "Full Time"}</p>
          </div>
          <div className="chartHeaders" style={{...itemStyle,background:"#5a9316"}}>
            <p>
              {sampleData
                ? "63.5%"
  ...
  : (typeof avgTenureHeader === 'number' && !isNaN(avgTenureHeader) 
    ...
    : "0")}
      ...
      <Panel
        ...
        isOpen={OpenPanel}
        onDismiss={() =>{ setOpenPanel(false);clearData()}}
                  ...
                  <div className={styles.RichEditorBlk}>
                    <div className="ms-Grid" dir="ltr">
                      ...
                      <div className="ms-Grid-row">
                        ...
                        <div className="ms-Grid-col ms-sm12 ms-md6 ms-lg6">
                          <Label>
                            {translation.Gender ? translation.Gender : "Gender"}
                          </Label>
