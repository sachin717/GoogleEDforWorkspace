...
const Dashboard = ({
  ...
  React.useEffect(() => {
    if (appSettings?.SyncUserInfoFrom == "Google & Imported User") {
      setSearchBoxData(appSettings?.AllUsersData?.GoogleUsers);
      employees = appSettings?.AllUsersData?.GoogleUsers;
    }
  }, []);
  ...
  const updateRecord = async () => {
    ...
    if (appSettings?.SyncUserInfoFrom == "Google & Imported User") {
      let user = appSettings?.AllUsersData?.GoogleUsers?.find((item) => {
        return item?.email == selectedEmail;
      });
      let userIndex = appSettings?.AllUsersData?.GoogleUsers?.findIndex(
        (item) => {
          return item?.email == selectedEmail;
        }
      );
      ...
      if (userIndex > -1) {
        let data = [...appSettings?.AllUsersData?.GoogleUsers];
  ...
  const saveallview = () => {
    let temp = {
      ...settingData,
      dashboardSettings: {
        newEmpByMonth: databyMonth,
        totalEmpBYDept: dept,
        empType: empType,
        avgAgeByDept: avg,
        avgAgeByLoc: agebylocation,
        avgTenByDept: avgTenByDept,
        avgTenByLoc: AvgTenByLoc,
        genDivByDept: genDivByDept,
        genDivByLoc: genDivByLoc,
      },
    };
  ...
  const saveDataBulkUsers = () => {
    ...
    if (updateDataArray?.length) {
      ...
      updateSettingData({ ...appSettings, AllUsersData: data });
      setAppSettings({ ...appSettings, AllUsersData: data });
      ...
      >
        <div style={containerStyle}>
          <div className="chartHeaders" style={{...itemStyle}}>
            ...
            <p>
              {translation.Head ? translation.Headcount : "countHead Count"}
            </p>
          </div>
          ...
          <div className="chartHeaders" style={{...itemStyle,background:"#5a9316"}}>
            <p>
              {sampleData
                ? "63.5%"
                : `${genderDiversityRatioForHeader.maleRatio}/${genderDiversityRatioForHeader.femaleRatio}`}
            </p>
            <p>
